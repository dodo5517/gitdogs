...
// 견종별 부품 로더 (parts/<breed>/ 아래에서 읽는다)
function partsDir(breed) {
  return path.join(__dirname, 'parts', breed);
}
function loadMeta(breed) {
  return JSON.parse(
    fs.readFileSync(path.join(partsDir(breed), 'meta.json'), 'utf8'),
  );
}
function loadFrag(breed, n) {
  return fs.readFileSync(path.join(partsDir(breed), `${n}.frag`), 'utf8');
}
const r = (x) => Number(x.toFixed(4));

// 세그먼트: { motion, dur(초), from:{x,y}, to:{x,y}, face? }
// from == to 이면 제자리 모션. 방향은 기본적으로 이동 방향을 따른다.
const seg = (motion, dur, from, to = from, face) => ({
  motion,
  dur,
  from,
  to,
...
function movement(from, to, k0, k1, total) {
  ...
  return {
    ...
    anim: `<animateTransform attributeName="transform" type="translate" values="${pts
      ...
      .join(
        '; ',
      )}" keyTimes="${pts.map(([k]) => r(k)).join('; ')}" dur="${total}s" repeatCount="indefinite"/>`,
  };
}

function composeScene(segments, breed = 'dachshund') {
  const META = loadMeta(breed);
  const frag = (n) => loadFrag(breed, n);
  ...
  if (first.from.x !== last.to.x || first.from.y !== last.to.y)
    console.warn(
      `[compose] 루프 미닫힘: 시작 (${first.from.x},${first.from.y}) / 끝 (${last.to.x},${last.to.y})`,
    );
  ...
  const body = segments
    .map((s) => {
      ...
      const face =
        s.face ?? (moving ? (s.to.x < s.from.x ? 'left' : 'right') : facing);
      facing = face;
      // 왼쪽을 볼 때는 부품 폭 기준으로 좌우 반전
      const flip =
        face === 'left' ? ` transform="translate(${w},0) scale(-1,1)"` : '';
      ...
      return [
        ...
        mov.open,
        mov.anim,
        `<g${flip}><use href="#part_${s.motion}"/></g>`,
        `</g>`,
        `</g>`,
      ].join('\n');
    })
...
const SCENES = {
  // 공통분모 12종만 사용 (beg/dig 제외) — 모든 견종에서 동일하게 동작
...
// 견종 전용 슬롯(play/react/observe)을 활용하는 장면.
// 슬롯의 실제 모션은 견종마다 다르지만 이름이 같아 장면 로직은 하나로 공용.
SCENES.playfulA = () => [
  seg('walk', 8, p(80, 0), p(160, -8)),
  seg('play', 6, p(160, -8), p(640, -8)), // 물고 저쪽으로 달려감
  seg('react', 8, p(640, -8)),
  seg('play', 6, p(640, -8), p(160, -8)), // 물고 돌아옴
  seg('walk', 6, p(160, -8), p(80, 0)),
];
SCENES.curiousA = () => [
  seg('walk', 11, p(120, 0), p(520, -14)),
  seg('observe', 10, p(520, -14)),
  seg('react', 8, p(520, -14)),
  seg('sniff', 9, p(520, -14), p(240, -6)),
  seg('walk', 7, p(240, -6), p(120, 0)),
];

const VARIANTS = {
  zoomies: ['zoomiesA', 'zoomiesB'],
  active: ['activeA', 'activeB', 'playfulA'],
  normal: ['normalA', 'normalB', 'normalC', 'curiousA'],
...
function sceneFor(state, seed = '', breed = 'dachshund') {
